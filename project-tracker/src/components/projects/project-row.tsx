...
export function ProjectRow({ project }: ProjectRowProps) {
  ...
  return (
    <tr className="border-b border-white/5 text-sm text-slate-100">
      <td className="py-4">
        <div className="font-medium text-white">{project.name}</div>
        <div className="text-xs text-slate-400">{project.customer?.name ?? "No customer"}</div>
      </td>
      <td className="py-4">
        <select
          value={status}
      ...
      <td className="py-4">
        {deadline ? (
          <div className={`text-xs ${overdue ? "text-rose-300" : "text-slate-300"}`}>
      ...
      <td className="py-4 text-center text-xs text-slate-300">
